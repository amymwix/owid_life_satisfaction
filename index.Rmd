...
happiness_chart <- 
  happiness %>% 
  filter(Country %in% c("Nigeria", "Latvia", "Puerto Rico", "Poland", "Turkey")) %>%
  ggplot(aes(x = Year, y = `Happiness: Happy (aggregate)`, color = Country)) +
  geom_line() +  # Draws lines
  geom_point() +  # Adds points to the lines
  scale_y_continuous(
    limits = c(0, 100),
    breaks = seq(0, 100, 20),  # Sets breaks at every 20% interval
    labels = scales::percent_format(scale = 1)  # Formats labels as percentages
  ) +
  scale_x_continuous(
    breaks = c(1993, 2000, 2005, 2010, 2015, 2022)  # Sets custom breaks for the x-axis
  ) +
  labs(x = NULL,
       y = NULL, 
       color = NULL) +
